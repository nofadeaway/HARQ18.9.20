...
public:
  ...
  void deallocate(byte_buffer_t *b)
  {
    ...
    if (!pool->deallocate(b))
    {
      if (log)
      {
        log->error("Deallocating PDU: Addr=0x%lx, name= NULL not found in pool\n", (uint64_t)b); //b->debugname
      }
      else
      {
        printf("Error deallocating PDU: Addr=0x%lx, name= NULL not found in pool\n", (uint64_t)b);
      }
    }
    b = NULL;
  }
  ...
  uint32_t get_allocate()   //FX create
  {
    return pool->nof_available_pdus();
  }
