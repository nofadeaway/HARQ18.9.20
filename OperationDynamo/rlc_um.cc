...
rlc_um::rlc_um() : tx_sdu_queue(16)
{
  ...
  pool = byte_buffer_pool::get_instance(); //new version FX
...
{
  static uint32_t functiontimes = 0;
  ++functiontimes;
  printf("Now functiontimes is %u. \n",functiontimes);
  ...
  printf("Now allocate is %u. pdu->N_bytes is %u.\n", pool->get_allocate(), pdu->N_bytes);
  if ((pdu == NULL) || pdu->N_bytes != 0)
  {
    if (pdu == NULL)
      printf("ptr pdu is null！\n");
    log->error("Failed to allocate PDU buffer\n");
    printf("Now allocate is %u.\n", pool->get_allocate());
    exit(1);
    return 0;
  }
...
//       pdu_lost = false;
//     }
//     printf("come here MARK 1\n");
//     // Handle last segment
//     printf("rx_window[vr_ur].buf->msg： rx_sdu->N_bytes is %u, rx_window[vr_ur].buf->N_bytes is %u.\n",rx_sdu->N_bytes),rx_window[vr_ur].buf->N_bytes;
//     memcpy(&rx_sdu->msg[rx_sdu->N_bytes], rx_window[vr_ur].buf->msg, rx_window[vr_ur].buf->N_bytes);
//     printf("come here MARK 2\n");
//     rx_sdu->N_bytes += rx_window[vr_ur].buf->N_bytes;
...
void rlc_um::reassemble_rx_sdus()
{
  ...
  while (!inside_reordering_window(vr_ur))
  {
    ...
    else
    {
      ...
      for (uint32_t i = 0; i < rx_window[vr_ur].header.N_li; i++)
      {
        ...
        {
          log->info_hex(rx_sdu->msg, rx_sdu->N_bytes, "Rx SDU vr_ur=%d, i=%d (lower edge middle segments)", vr_ur, i);
          rx_sdu->reset();
          pool->deallocate(rx_sdu);   // added by FX 9.11
          // if(cfg.is_mrb){
          //   pdcp->write_pdu_mch(lcid, rx_sdu);     //pdcp->write_pdu中内涵了deallocate
          // } else {
          //   pdcp->write_pdu(lcid, rx_sdu);
          // }
          rx_sdu = pool->allocate();
      ...
      if (rx_sdu->N_bytes > 0 || rlc_um_start_aligned(rx_window[vr_ur].header.fi))
      {
        ...
        if (rlc_um_end_aligned(rx_window[vr_ur].header.fi))
        {
          ...
          {
            log->info_hex(rx_sdu->msg, rx_sdu->N_bytes, "Rx SDU vr_ur=%d (lower edge last segments)", vr_ur);
            rx_sdu->timestamp = bpt::microsec_clock::local_time();
            // if(cfg.is_mrb){
      ...
      // Clean up rx_window
      pool->deallocate(rx_window[vr_ur].buf);  //rx_window的allocate是在 rlc_um::handle_data_pdu()
      rx_window.erase(vr_ur);
    }
  ...
  while (rx_window.end() != rx_window.find(vr_ur))
  {
    ...
    for (uint32_t i = 0; i < rx_window[vr_ur].header.N_li; i++)
    {
      ...
      {
        log->info_hex(rx_sdu->msg, rx_sdu->N_bytes, " Rx SDU vr_ur=%d, i=%d, (update vr_ur middle segments)", vr_ur, i);
        rx_sdu->timestamp = bpt::microsec_clock::local_time();
        // if(cfg.is_mrb){
        //   pdcp->write_pdu_mch(lcid, rx_sdu);
        // } else {
    ...
    if (rlc_um_end_aligned(rx_window[vr_ur].header.fi))
    {
      ...
      {
        log->info_hex(rx_sdu->msg, rx_sdu->N_bytes, "Rx SDU vr_ur=%d (update vr_ur last segments)", vr_ur);
        rx_sdu->timestamp = bpt::microsec_clock::local_time();
        // if(cfg.is_mrb){
        //   pdcp->write_pdu_mch(lcid, rx_sdu);
        // } else {
