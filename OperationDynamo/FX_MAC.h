...
public:
  ...
  pthread_mutex_t qbuff_LOCK; //实现harq_busy()从qbuff队列中取数据，和pdu_in写数据的互斥
  ...
  bool ACK[HARQ_NUM] = {0};
  bool ACK_default[HARQ_NUM] = {0};
  bool busy_index[HARQ_NUM] = {0};
  uint32_t new_trans_index[HARQ_NUM] = {};
  uint32_t has_been_trans[HARQ_NUM] = {};
  uint32_t retrans_overflow_times = 0;
  ...
  bool pdu_store(uint32_t pid_now, uint8_t *payload_back, uint32_t pdu_sz_test)
  {
    ...
    qbuff_flag = pdu_queue_test.pdu_q[pid_now].send(payload_back, pdu_sz_test); //把payload)back存入qbuff,send函数不判断满
    if (qbuff_flag)
    {
  ...
  bool pdu_in(uint8_t *payload_back, uint32_t pdu_sz_test) // 将pdu存入为空闲状态的HARQ进程的队列，但只查找一次
  {
    if (payload_back != NULL)
    {
      ...
      {
        while ((busy_index[pid_not_busy] || pdu_queue_test.pdu_q[pid_not_busy].isfull()) && (times < HARQ_NUM)) //找一个不忙也不满的队列  !!!大哥这个次数16次显然应该是跟前面的与(我怎么能写成或...)
        {
      ...
      if (!(pdu_queue_test.pdu_q[pid_not_busy].isfull()))
      {
        suc = pdu_store(pid_not_busy, payload_back, pdu_sz_test);
      }
      else
        suc = false;
      if (flag)
      ...
      pthread_mutex_unlock(&qbuff_LOCK);
      return suc;
    }
    else
    {
      printf("pdu_in:::PTR is NULL!\n");
      return false;
    }
  }

  bool all_qbuff_full() //判断是否还有空的harq的队列，若全为满，返回true
  {
    for (int i = 0; i < HARQ_NUM; ++i)
    {
      if (!(pdu_queue_test.pdu_q[i].isfull()))
        return false;
    }
    return true;
  }
  ...
  int set_busy(uint32_t pid) //将一个PID状态变为忙，用于测试的函数
  {
    pthread_mutex_lock(&busy_LOCK);
    busy_index[pid] = true;
    busy_num++;
  ...
  bool is_new_trans(uint32_t pid_now)
  {
    return (pdu_queue_test.pdu_q[pid_now].rp_is() == new_trans_index[pid_now]);
  }

  int not_new_trans(uint32_t pid_now)
  {
    if (pdu_queue_test.pdu_q[pid_now].isempty())
    {
      new_trans_index[pid_now] = pdu_queue_test.pdu_q[pid_now].wp_is();
    }
    else
    {
      new_trans_index[pid_now] = (pdu_queue_test.pdu_q[pid_now].rp_is() + 1) % QBUFF_PDUS;
    }
  }

  uint8_t *trans_control(uint32_t pid_now, uint32_t *len)
  {
    return trans_control(pid_now, len, NULL);
  }

  uint8_t *trans_control(uint32_t pid_now, uint32_t *len, uint32_t *number)
  {
    int retrans_limit = 5;
    static int retrans_times[HARQ_NUM] = {0};
    //begin{5.29}
    ...
    if (is_new_trans(pid_now)) //对于初传，没有对应的ACK,所以不看ACK数值
    {
      //若是初传，直接发送，不看ACK值
      printf("RNTI:%d:::NEW:::Now PID No.%d:queue's No.%d buffer will be sent.\n", rnti, pid_now, pdu_queue_test.pdu_q[pid_now].rp_is()); //接收到ACK,发送下一个PDU
      retrans_times[pid_now] = 0;                                                                                                         //判定为新传，把重传次数变为0
      //return (uint8_t *)pdu_queue_test.pdu_q[pid_now].pop(&len);
    }
    else
    {
      if (ACK_temp) //若为重传，要查看ACK数值
      {
        ...
        retrans_times[pid_now] = 0;                                                                                                    //收到ACK，release(),清空retrans次数
        // *number = pdu_queue_test.pdu_q[pid_now].rp_is();
        // return (uint8_t *)pdu_queue_test.pdu_q[pid_now].pop(&len);
      }
      else //没收到ACK,重发
      {
        ...
        if (retrans_times[pid_now] < retrans_limit)
        {
          ...
          pdu_queue_test.pdu_q[pid_now].pop(len);
  ...
  bool init(phy_interface_mac *phy_h_, rlc_interface_mac *rlc_, srslte::log *log_h_, bsr_proc *bsr_procedure_, phr_proc *phr_procedure_, pdu_queue::process_callback *callback_)
  {
    ...
    pthread_mutex_init(&qbuff_LOCK, NULL);
  ...
  {
    return harq_busy(pid, len, NULL);
  }

  uint8_t *harq_busy(uint32_t *pid, uint32_t *len, uint32_t *number) //用于物理层发送，发送一个不出于busy状态的harq进程的pdu,这个重载版本可以得到发送的编号
  {
    pthread_mutex_lock(&qbuff_LOCK);
    pthread_mutex_lock(&busy_LOCK);
    uint8_t *pay_load = NULL;
    uint32_t times = 0; //防止全空的时候无限循环
    printf("Now busy number is %d.\n", busy_num);
    if (busy_num < HARQ_NUM) //这里我居然忘记了解锁，我真是太蠢了！！！
    {
      uint32_t pid_not_busy = 0;
      while ((busy_index[pid_not_busy] || pdu_queue_test.pdu_q[pid_not_busy].isempty()) && (times < HARQ_NUM)) //寻找一个即不处于繁忙状态，也非空的harq进程号
      {
        pid_not_busy = (pid_not_busy + 1) % HARQ_NUM;
        ++times;
      }
      if (!(pdu_queue_test.pdu_q[pid_not_busy].isempty()))
      {
        pay_load = trans_control(pid_not_busy, len, number);
        if (pay_load != NULL) //如果为空，trans_control返回NULL
        {
      ...
      {
        printf("harq_busy():All queue is empty!No data!\n");
      }
    }
    pthread_mutex_unlock(&busy_LOCK);
    pthread_mutex_unlock(&qbuff_LOCK);
  ...
  int set_ack_come(uint32_t subframe_now, bool *ack_) //通过映射表，找出当前这个子帧收到的ACK对应前面哪个子帧,ack_指向bool数组，用于一个上行帧回复多个下行帧
  {
    pthread_mutex_lock(&ACK_LOCK);
    int flag = -1, offset = 0;
    for (int i = 0; i < NUM_SUBFRAME; ++i)
    {
      if (ack_map[i].ack_come == subframe_now)
      {
        if (ack_map[i].valid)
        {
          ack_recv(ack_map[i].harq_pid);
          ack_map[i].valid = false;
          ACK[ack_map[i].harq_pid] = *(ack_ + offset);
          flag = 1;
          ++offset;
        }
      }
...
public:
  ...
  int subframe_process_now() //告知调用此函数者，当前子帧属性，上行/下行/切换
  {
    ...
    {
      return downlink_subframe;
    }
    else if (subframe_number != 1) //1号子帧为切换帧
    {
      return uplink_subframe;
    }
    else
      return swith_subframe;
  }
